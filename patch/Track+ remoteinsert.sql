...
--Taken care of GST rates 18% and 28%
  ...
  if @ptype = 'I' then //insert
    ...
    if @gst18 is not null then
        if @isAdvance = 'y' then
            insert into cash_receipt (acc_id, acc_id_cash, rec_date,ref_no, remarks, amt)
                values (@acc_id, @acc_id_cash, @date, @rec_no, 'Job No:'+@job_no+ ' Advance receipt from service+', @rec_amt );
        else
            insert into bill_memo( type,bill_memo,ref_no,"date",sale_tax_sale_id,
              total_amt,acc_id,descr,mpreserve, cgst, sgst, igst, gstin ) 
              values( @rectype,'M',@rec_no,@date,@gst18,@rec_amt,@acc_id_cash,
                'Job No:'+@job_no + ':Invoice:' + rtrim(@other_info),'N', @cgst, @sgst, @igst, @gstin );
        end if;
     end if;
  else
    //if @isAdvance = 'y' then
        delete from cash_receipt where ref_no = @rec_no;
    //else
